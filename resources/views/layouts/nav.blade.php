@php
    ...
    $link = fn ($active) => 'nav-link'.($active ? ' nav-link-active' : '');
@endphp
<div class="flex h-full flex-col">
    ...
    @if ($user->isPatient())
        ...
        <nav class="mt-4 flex-1 space-y-1">
            <a href="{{ route('portal.record') }}" class="{{ $link(request()->routeIs('portal.*')) }}">
    ...
    @else
        <nav class="mt-4 flex-1 space-y-1">
            @foreach ($nav as $item)
                ...
                <a href="{{ route($item['route']) }}" class="{{ $link($active) }}">
            ...
            <a href="{{ route('invoices.index') }}" class="{{ $link(request()->routeIs('invoices.*')) }}">
            ...
            <div class="mt-4 border-t border-white/10 pt-4">
                <div class="px-3 pb-1 text-xs font-semibold uppercase tracking-wider text-brand-300/80">Amministrazione</div>
                <a href="{{ route('users.index') }}" class="{{ $link(request()->routeIs('users.*')) }}">
                ...
                <a href="{{ route('audit.index') }}" class="{{ $link(request()->routeIs('audit.*')) }}">
                ...
                @if ($user->isSuperAdmin())
                    <a href="{{ route('settings.edit') }}" class="{{ $link(request()->routeIs('settings.*')) }}">
                    ...
                    <a href="{{ route('integrations.edit') }}" class="{{ $link(request()->routeIs('integrations.*')) }}">
        ...
        @if (app(\App\Services\GoogleCalendarService::class)->isConfigured())
            <div class="border-t border-white/10 pt-3">
                <a href="{{ route('google.redirect') }}" class="nav-link">
                    <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
